...
async function getDashboardData()  {
    ...
    try {
        ...
        const responseProperty = await fetch('http://localhost:8081/api/property/getAll', {
            method: 'GET',
            headers: {
                'Content-Type': 'application/json',
        ...
        const responseTransaction = await fetch('http://localhost:8081/api/transaction/getAll', {
            method: 'GET',
            headers: {
                'Authorization': 'Bearer ' + token,
                'Content-Type': 'application/json'
...
function totalAgent(users) {
    const agentCount = users.filter(user =>
        user.roles.some(role => role.name === 'AGENT')
    ).length;

    const agentElement = document.getElementById('totalAgents');
    if (agentElement) {
        agentElement.textContent = agentCount;
    } else {
        console.error("Phần tử 'totalCustomers' không tồn tại.");
    }
}
